...
const ICON_VARIANTS: Record<CoreAnimation, Variants> = {
  ...
  roll: {
    initial: { opacity: 0, y: 'var(--space-xs)', filter: ROLL_BLUR },
    animate: {
      opacity: 1,
      y: 0,
      filter: 'blur(0px)',
...
const SIZE_CLASS: Record<ActionSwapButtonSize, string> = {
  sm: 'h-lg gap-2xs rounded-md px-xs text-[length:var(--text-caption)] leading-meta',
  md: 'h-xl gap-2xs rounded-md px-sm text-[length:var(--text-meta)] leading-meta',
  lg: 'h-2xl gap-xs rounded-md px-md text-[length:var(--text-body)] leading-heading',
  icon: 'size-xl rounded-md',
};
...
export function ActionSwapButton({
  ...
  return (
    <motion.button
      ...
      className={cn(
        'ui-scope inline-flex items-center justify-center overflow-hidden font-[var(--font-weight-medium)] transition-colors focus-visible:outline-2 focus-visible:outline-ring [outline-offset:var(--space-3xs)]',
